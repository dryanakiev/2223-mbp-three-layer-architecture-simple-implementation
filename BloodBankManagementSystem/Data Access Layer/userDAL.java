namespace BloodBankManagementSystem.DAL {
    ...
    class userDAL {
        ...
        public final boolean Insert(userBLL u) {
        ...
        public final boolean Update(userBLL u) {
            // Create a Boolean variable and set its default value to false
            boolean isSuccess = false;
            ...
            try {
                ...
                string sql = "UPDATE tbl_users SET username=@username, email=@email, password=@password, full_name=@full_name, cont" +
                        "act=@contact, address=@address, added_date=@added_date, image_name=@image_name WHERE user_id=@user_i" +
                        "d";
        ...
        public final boolean Delete(userBLL u) {
            // Create a boolean variable and set its default value to false
            boolean isSuccess = false;
        ...
        public final DataTable Search(string keywords) {
            ...
            try {
                ...
                String sql = ("SELECT * FROM tbl_users WHERE user_id LIKE \'%"
                        + (keywords + ("%\' OR full_name LIKE \'%"
                        + (keywords + ("%\' OR address LIKE \'%"
                        + (keywords + "%\'"))))));
                // Create SQL Command to Execute the Query
                SqlCommand cmd = new SqlCommand(sql, conn);
        ...
        SqlDataAdapter adapter = new SqlDataAdapter(this.sql, this.conn);
    }
}