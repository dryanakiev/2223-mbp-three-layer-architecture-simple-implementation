namespace BloodBankManagementSystem.DAL {
    ...
    class donorDAL {
        ...
        public final boolean Insert(donorBLL d) {
            ...
            try {
                ...
                string sql = "INSERT INTO tbl_donors (first_name, last_name, email, contact, gender, address, blood_group, added_da" +
                        "te, image_name, added_by) VALUES (@first_name, @last_name, @email, @contact, @gender, @address, @blo" +
                        "od_group, @added_date, @image_name, @added_by)";
        ...
        public final boolean Update(donorBLL d) {
            // Create a Boolean Variable and SEt its Default Value to FAlse
            boolean isSuccess = false;
            // Create SQLConnection to Connect DAtabase
            SqlConnection conn = new SqlConnection(myconnstrng);
            try {
                ...
                string sql = "UPDATE tbl_donors SET first_name=@first_name, last_name=@last_name, email=@email, contact=@contact, g" +
                        "ender=@gender, address=@address, blood_group=@blood_group, image_name=@image_name, added_by=@added_b" +
                        "y WHERE donor_id=@donor_id";
        ...
        public final boolean Delete(donorBLL d) {
            // Create a Boolean variable and set its default value to false
            boolean isSuccess = false;
            // Create SqlConnection To Connect DAtabase
            SqlConnection conn = new SqlConnection(myconnstrng);
        ...
        public final string countDonors(string blood_group) {
            ...
            try {
                ...
                string sql = ("SELECT * FROM tbl_donors WHERE blood_group = \'"
                        + (blood_group + "\'"));
            ...
            public final DataTable Search(string keywords) {
                ...
                try {
                    ...
                    string sql = ("SELECT * FROM tbl_donors WHERE donor_id LIKE \'%"
                            + (keywords + ("%\' OR first_name LIKE \'%"
                            + (keywords + ("%\' OR last_name LIKE \'"
                            + (keywords + ("\' OR email LIKE \'%"
                            + (keywords + ("%\' OR blood_group LIKE \'"
                            + (keywords + "\'"))))))))));
                    // Create SQL Command to Execute the Query
                    SqlCommand cmd = new SqlCommand(sql, conn);
